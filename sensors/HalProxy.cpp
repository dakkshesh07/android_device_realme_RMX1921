...
typedef V2_0::implementation::ISensorsSubHal *(
    SensorsHalGetSubHalFunc)(uint32_t *);
typedef V2_1::implementation::ISensorsSubHal *(
    SensorsHalGetSubHalV2_1Func)(uint32_t *);
...
int32_t setSubHalIndex(int32_t sensorHandle, size_t subHalIndex) {
  return sensorHandle |
         (static_cast<int32_t>(subHalIndex) << kBitsAfterSubHalIndex);
}
...
HalProxy::~HalProxy() { stopThreads(); }

Return<void>
HalProxy::getSensorsList_2_1(ISensorsV2_1::getSensorsList_2_1_cb _hidl_cb) {
...
Return<void>
HalProxy::getSensorsList(ISensorsV2_0::getSensorsList_cb _hidl_cb) {
...
Return<Result> HalProxy::initialize_2_1(
    const ::android::hardware::MQDescriptorSync<V2_1::Event>
        &eventQueueDescriptor,
  ...
  // positions.
  auto hidlWakeLockQueue = std::make_unique<WakeLockMessageQueue>(
      wakeLockDescriptor, true /* resetPointers */);
...
Return<Result> HalProxy::initialize(
    const ::android::hardware::MQDescriptorSync<V1_0::Event>
        &eventQueueDescriptor,
    const ::android::hardware::MQDescriptorSync<uint32_t> &wakeLockDescriptor,
  ...
  // Create the Event FMQ from the eventQueueDescriptor. Reset the read/write
  // positions.
  auto eventQueue = std::make_unique<EventMessageQueueV2_0>(
      eventQueueDescriptor, true /* resetPointers */);
  std::unique_ptr<EventMessageQueueWrapperBase> queue =
...
Return<Result> HalProxy::initializeCommon(
  ...
  mPendingWriteEventsQueue =
      std::queue<std::pair<std::vector<V2_1::Event>, size_t>>();
  ...
  if (EventFlag::createEventFlag(mWakeLockQueue->getEventFlagWord(),
                                 &mWakelockQueueFlag) != OK) {
    result = Result::BAD_VALUE;
  }
  if (!mDynamicSensorsCallback || !mEventQueue || !mWakeLockQueue ||
...
Return<Result> HalProxy::batch(int32_t sensorHandle, int64_t samplingPeriodNs,
  ...
  return getSubHalForSensorHandle(sensorHandle)
      ->batch(clearSubHalIndex(sensorHandle), samplingPeriodNs,
              maxReportLatencyNs);
}
...
Return<Result> HalProxy::flush(int32_t sensorHandle) {
  ...
  return getSubHalForSensorHandle(sensorHandle)
      ->flush(clearSubHalIndex(sensorHandle));
}
...
Return<void> HalProxy::registerDirectChannel(
    const SharedMemInfo &mem, ISensorsV2_0::registerDirectChannel_cb _hidl_cb) {
...
Return<void>
HalProxy::configDirectReport(int32_t sensorHandle, int32_t channelHandle,
                             RateLevel rate,
  ...
  if (mDirectChannelSubHal == nullptr) {
    ...
    mDirectChannelSubHal->configDirectReport(sensorHandle, channelHandle, rate,
                                             _hidl_cb);
  }
  return Return<void>();
}

Return<void> HalProxy::debug(const hidl_handle &fd,
  ...
  stream << "  Threads are running: " << (mThreadsRun.load() ? "true" : "false")
         << std::endl;
  int64_t now = getTimeNow();
  stream << "  Wakelock timeout start time: "
         << msFromNs(now - mWakelockTimeoutStartTime) << " ms ago" << std::endl;
  stream << "  Wakelock timeout reset time: "
         << msFromNs(now - mWakelockTimeoutResetTime) << " ms ago" << std::endl;
  // TODO(b/142969448): Add logging for history of wakelock acquisition per
  // subhal.
  stream << "  Wakelock ref count: " << mWakelockRefCount << std::endl;
  stream << "  # of events on pending write writes queue: "
         << mSizePendingWriteEventsQueue << std::endl;
  ...
  stream << "  # of non-dynamic sensors across all subhals: " << mSensors.size()
         << std::endl;
  stream << "  # of dynamic sensors across all subhals: "
         << mDynamicSensors.size() << std::endl;
      ...
      if (handle == nullptr) {
        ...
        if (sensorsHalGetSubHalPtr != nullptr) {
          ...
          SensorsHalGetSubHalV2_1Func *getSubHalV2_1Ptr =
              (SensorsHalGetSubHalV2_1Func *)dlsym(handle,
                                                   "sensorsHalGetSubHal_2_1");

          if (getSubHalV2_1Ptr == nullptr) {
            ALOGE(
                "Failed to locate sensorsHalGetSubHal function for library: %s",
                subHalLibraryFile.c_str());
          } else {
            ...
            if (version != SUB_HAL_2_1_VERSION) {
              ...
              ALOGV("Loaded SubHal from library: %s",
                    subHalLibraryFile.c_str());
              mSubHalList.push_back(
                  std::make_unique<SubHalWrapperV2_1>(subHal));
            }
          }
        }
      }
...
void HalProxy::initializeSensorList() {
  for (size_t subHalIndex = 0; subHalIndex < mSubHalList.size();
       subHalIndex++) {
    auto result =
        mSubHalList[subHalIndex]->getSensorsList([&](const auto &list) {
          for (SensorInfo sensor : list) {
            if (!subHalIndexIsClear(sensor.sensorHandle)) {
              ...
              sensor.sensorHandle =
                  setSubHalIndex(sensor.sensorHandle, subHalIndex);
...
void HalProxy::init() { initializeSensorList(); }
...
void HalProxy::handlePendingWrites() {
  ...
  while (mThreadsRun.load()) {
    mEventQueueWriteCV.wait(lock, [&] {
      return !mPendingWriteEventsQueue.empty() || !mThreadsRun.load();
    });
    if (mThreadsRun.load()) {
      std::vector<Event> &pendingWriteEvents =
          mPendingWriteEventsQueue.front().first;
...
void HalProxy::handleWakelocks() {
  ...
  while (mThreadsRun.load()) {
    mWakelockCV.wait(
        lock, [&] { return mWakelockRefCount > 0 || !mThreadsRun.load(); });
    if (mThreadsRun.load()) {
      int64_t timeLeft;
...
void HalProxy::postEventsToMessageQueue(
    const std::vector<Event> &events, size_t numWakeupEvents,
  ...
  if (numToWrite < events.size() && mSizePendingWriteEventsQueue + numLeft <=
                                        kMaxSizePendingWriteEventsQueue) {
    std::vector<Event> eventsLeft(events.begin() + numToWrite, events.end());
    mPendingWriteEventsQueue.push({eventsLeft, numWakeupEvents});
    mSizePendingWriteEventsQueue += numLeft;
    mMostEventsObservedPendingWriteEventsQueue =
        std::max(mMostEventsObservedPendingWriteEventsQueue,
                 mSizePendingWriteEventsQueue);
    mEventQueueWriteCV.notify_one();
  }
}

bool HalProxy::incrementRefCountAndMaybeAcquireWakelock(
    size_t delta, int64_t *timeoutStart /* = nullptr */) {
  if (!mThreadsRun.load())
    return false;
  std::lock_guard<std::recursive_mutex> lockGuard(mWakelockMutex);
...
void HalProxy::decrementRefCountAndMaybeReleaseWakelock(
    size_t delta, int64_t timeoutStart /* = -1 */) {
  if (!mThreadsRun.load())
    return;
  std::lock_guard<std::recursive_mutex> lockGuard(mWakelockMutex);
  if (delta > mWakelockRefCount) {
    ALOGE("Decrementing wakelock ref count by %zu when count is %zu", delta,
          mWakelockRefCount);
  }
  if (timeoutStart == -1)
    timeoutStart = mWakelockTimeoutResetTime;
  if (mWakelockRefCount == 0 || timeoutStart < mWakelockTimeoutResetTime)
    return;
...
void HalProxy::setDirectChannelFlags(
  ...
  } else if (mDirectChannelSubHal != nullptr &&
             subHal != mDirectChannelSubHal) {
...
std::shared_ptr<ISubHalWrapperBase>
...
size_t HalProxy::countNumWakeupEvents(const std::vector<Event> &events,
                                      size_t n) {
  size_t numWakeupEvents = 0;
  for (size_t i = 0; i < n; i++) {
    int32_t sensorHandle = events[i].sensorHandle;
    if (mSensors[sensorHandle].flags &
        static_cast<uint32_t>(V1_0::SensorFlagBits::WAKE_UP)) {
      numWakeupEvents++;
    }
  }
